...
#define PREFIX "[tmpfs]"
#define info(fmt, ...) printk(PREFIX " " fmt, ##__VA_ARGS__)
#if 0
#define debug(fmt, ...) \
	printk(PREFIX "<%s:%d>: " fmt, __func__, __LINE__, ##__VA_ARGS__)
#else
#define debug(fmt, ...) do { } while (0)
#endif
#define warn(fmt, ...) printk(PREFIX " " fmt, ##__VA_ARGS__)
#define error(fmt, ...) printk(PREFIX " " fmt, ##__VA_ARGS__)
